...
$sql = "Select ListingID, Name, archived From (Select 1) as ignore_me left join $input on 1 = 1 Limit 1";
...
$sql = "Select ListingID, Name, archived from ".$input ." where PosterID = ?";
if(isset($conn)){
$result = $conn -> execute_query($sql, [$userid]);
$numfields = mysqli_num_fields($result);

echo("<div class = 'table'><div><div></div>");
...
while($row = $result -> fetch_row()){
	echo("<form action = 'php/toggleArchived.php' method = 'POST'>");
	...
	echo("<div><input type = 'submit' name = 'submit' value = 'Toggle Archived'></div></form>");
}
echo("</div>");
}
